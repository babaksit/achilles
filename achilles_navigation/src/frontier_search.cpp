...
FrontierSearch::FrontierSearch(ros::NodeHandle *nh,costmap_2d::Costmap2D* costmap):nh_(nh)
  ,costmap_(costmap)
{
}

std::list<frontier_exploration::Frontier> FrontierSearch::searchFrom(geometry_msgs::Point position){

    std::list<frontier_exploration::Frontier> frontier_list;
...
//    ROS_ERROR("size_x_ %d, size_y_ %d" , size_x_ , size_y_);
    ...
    if(nearestCell(clear, pos, FREE_SPACE, *costmap_)){
        bfs.push(clear);
//        ROS_WARN(" find nearby clear cell to start search %d" , clear);
    }else{
        bfs.push(pos);
...
frontier_exploration::Frontier FrontierSearch::buildNewFrontier(unsigned int initial_cell, unsigned int reference, std::vector<bool>& frontier_flag){
